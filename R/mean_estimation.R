...
estimate_mean_fun <- function(bf, bf_covariates, method, save_model_mean,
  ...
  # If knots are supplied
  kn <- if(is.null(knot[[1]])) NULL else {
    list(t = knot[[1]])
  }
  ...
  ############
  if(use_bam == TRUE){
    gam1 <- try(bam(pred, data = dat_help, method = method, knots = kn))
  }else{
    gam1 <- try(gam(pred, data = dat_help, method = method, knots = kn))
  }
